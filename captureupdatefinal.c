...
struct timespec 	starttime, RGBtime, RGBStime, Greytime;
unsigned int 		count;
//#define NUM (frame_count*2)

pthread_t threads[181];
...
unsigned char bigbuffer1[(1280*960)];
unsigned char bigbuffer2[(1280*960)];
struct v4l2_buffer buf;
struct timespec frame_time, frame_exit;
int deadmiss = 0;


static void *process_imageGrey(void *threadp)
{
    int i, newi, newsize=0;
    struct timespec frame_time;
    int y_temp, y2_temp, u_temp, v_temp;
    unsigned char *pptr1 = (unsigned char *)buffers[buf.index].start;
	...
	printf("Dump YUYV converted to YY size %d\n", buf.bytesused);
        ...
        //
        for(i=0, newi=0; i<buf.bytesused; i=i+4, newi=newi+2)
        {
            // Y1=first byte and Y2=third byte
            bigbuffer1[newi]=pptr1[i];
            bigbuffer1[newi+1]=pptr1[i+2];
        }
  
        dump_pgm(bigbuffer1, (buf.bytesused/2), framecnt, &frame_time);

	//pthread_exit(NULL);
}
static void *process_imageRGB(void *threadp)
{
    int i, newi, newsize=0;
    struct timespec frame_time;
    int y_temp, y2_temp, u_temp, v_temp;
   ...
   // printf("%d\n", buf.bytesused);
	...
	//	printf("%d", i+6);
            ...
            yuv2rgb(y_temp, u_temp, v_temp, &bigbuffer2[newi], &bigbuffer2[newi+1], &bigbuffer2[newi+2]);
            yuv2rgb(y2_temp, u_temp, v_temp, &bigbuffer2[newi+3], &bigbuffer2[newi+4], &bigbuffer2[newi+5]);
        }
	dump_ppm(bigbuffer2, ((size*6)/4), framecnt, &frame_time);
...
static void process_image(const void *p, int size)
{
    ...
    printf("frame %d: ", framecnt);
        ...
        pthread_create(&threads[framecnt], &sched_attr, process_imageRGB, NULL);
        pthread_create(&threads[framecnt+frame_count], &sched_attr , process_imageGrey, NULL);

    pthread_join(threads[framecnt], NULL);
	
    clock_gettime(CLOCK_REALTIME, &frame_exit);
   
    i = ((frame_exit.tv_sec*1000 + frame_exit.tv_nsec/1000000) - (frame_time.tv_sec*1000 + frame_time.tv_nsec/1000000));
    if (i  >= 50) deadmiss++;
    syslog(LOG_INFO, "RGB done in %d msec", i);

    pthread_join(threads[framecnt+frame_count], NULL);
    clock_gettime(CLOCK_REALTIME, &frame_exit);
    i = ((frame_exit.tv_sec*1000 + frame_exit.tv_nsec/1000000) - (frame_time.tv_sec*1000 + frame_time.tv_nsec/1000000));
    if (i >= 50) deadmiss++;
    syslog(LOG_INFO, "Greyscale done in %d msec", i);
...
//	printf("count %d\n", count); 
	...
	FLOAT temp;
	int j = 0;frame_count*2
	
   	 for(i=0; i<IMG_HEIGHT*IMG_WIDTH; i=i+3)
   	 {
    	   	R[i] = bigbuffer[i];
   	     G[i+1] = bigbuffer[i+1];
    	    B[i+2] = bigbuffer[i+2];
    ...
    {
       	bigbuffer[i] = convR[i];
        bigbuffer[i+1] = convG[i+1];
        bigbuffer[i+2] = convB[i+2];
    }
	}
...
static void mainloop(void)
{
   // unsigned int count;
    ...
    while (count > 0)
    {
	    ...
	    /*if(count == 60)
	    {
		clock_gettime(CLOCK_REALTIME, &RGBtime);
	    }

	    if(count == (frame_count/3))
	    {
		clock_gettime(CLOCK_REALTIME, &RGBStime);
	    }
*/
	...
	clock_gettime(CLOCK_REALTIME, &Greytime);
...
static void stop_capturing(void)
{
        ...
        switch (io) {
        case IO_METHOD_READ:
                /* Nothing to do. */clock_gettime(CLOCK_REALTIME, &RGBStime);
...
int main(int argc, char **argv)
{
    ...
    printf("\nNumber of 50msec deadlines missed = %d\n", deadmiss);
